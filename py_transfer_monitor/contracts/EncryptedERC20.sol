...
import "./EncryptedErrors.sol";

contract EncryptedERC20 is Reencrypt, EncryptedErrors, OracleCaller {
    enum ErrorCodes {
        NO_ERROR,
    ...
    // bytes32 public _cross_chain_pubkey;
    ...
    constructor(
        ...
        _name = name_;
        _symbol = symbol_;
        _cross_chain_receiver = msg.sender;
    }
    ...
    function callbackAmountUint64(uint256 requestID, uint64 amount) public onlyOracle{
        uint256[] memory params = getParamsUint(requestID);
        unchecked {
            address from = address(uint160(params[0]));
            address to = address(uint160(params[1]));
            emit CrossChainTransfer(amount, from, to);
        }
    }
